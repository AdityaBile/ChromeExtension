...
async function toggleProxyUIOn() {
    ...
    promisify(r => chrome.extension.sendMessage({ greeting: "enableProxy", url:host }, r))//CONTINUE
...
function toggleProxyUIOff() {
    ...
    promisify(r => chrome.extension.sendMessage({ greeting: "disableProxy" }, r))//CONTINUE
...
async function loadUI() {
    ...
    function toggleWhitelistedSiteOn() {
        return promisify(r => chrome.extension.sendMessage({ greeting: "toggleWhitelistedSite", value: true, url: host }, r))
    }
    function toggleWhitelistedSiteOff() {
        return promisify(r => chrome.extension.sendMessage({ greeting: "toggleWhitelistedSite", value: false, url: host }, r))
    }
    // #agad - whitelisted sites
    document.querySelector("#link-whitelisted-sites")?.addEventListener("click", () => { chrome.tabs.create({ url: "options.html" }); });

    let v = await promisify(r => chrome.extension.sendMessage({ greeting: "isWhitelistedSite", url: host }, r))

    updateToggleButton("proxyAllowed", !v);

    document.querySelector("#proxyAllowed .toggle-off").addEventListener("click", function () {
        updateToggleButton("proxyAllowed", false);
        toggleWhitelistedSiteOn();
    })

    document.querySelector("#proxyAllowed .toggle-on").addEventListener("click", function () {
